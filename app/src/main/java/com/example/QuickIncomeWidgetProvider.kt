...
class QuickIncomeWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        fun updateAppWidgets(context: Context, appWidgetManager: AppWidgetManager, appWidgetIds: IntArray) {
            ...
            for (appWidgetId in appWidgetIds) {
                val views = RemoteViews(context.packageName, R.layout.widget_quick_income)

                // 1. Tapping anywhere on the pill or "+" button launches QuickEntryActivity in Income mode
                ...
                views.setOnClickPendingIntent(R.id.widget_quick_root, incomePendingIntent)
                views.setOnClickPendingIntent(R.id.btn_quick_pill, incomePendingIntent)
                views.setOnClickPendingIntent(R.id.btn_quick_add, incomePendingIntent)
                ...
                val formattedAmount = DedunWidgetProvider.formatLkr(Math.abs(dailyIncome))

                views.setTextViewText(R.id.tv_quick_label, "TODAY · INCOME")
                ...
                // 3. Left side icon badge: arrow-down icon tinted with active theme accent color
        ...
        fun updateAllWidgets(context: Context) {
            ...
            val ids = appWidgetManager.getAppWidgetIds(ComponentName(context, QuickIncomeWidgetProvider::class.java))
            if (ids.isNotEmpty()) {
